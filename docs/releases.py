...
def get_latest_release(releases: List[Release]) -> dict:
...
def get_windows_exe(release: Release) -> Content:
    return next((f for f in release.files if f.os == 'Windows' and f.kind == 'Installer'), None)


def get_linux_exe(release: Release) -> Content:
    return next((f for f in release.files if f.os == 'Linux' and f.kind == 'AppImage'), None)


def get_macos_exe(release: Release) -> Content:
    return next((f for f in release.files if f.os == 'MacOS' and f.kind == 'Apple Disk Image'), None)
...
def main():
    ...
    endpoint_url = get_env('AWS_ENDPOINT_URL')
    s3 = session.client(
        service_name='s3',
