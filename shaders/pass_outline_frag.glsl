...
void main() {
    // Source: https://github.com/kiwipxl/GLSL-shaders/blob/master/outline.glsl
    // Slightly modified to suit the game needs.
    ...
    float sum = 0.0;
    for (int n = 0; n < 9; ++n) {
        stepY = (vs_out.texCoords.y * size.y) + (uniforms.thickness * float(n - 4.5));
        float h_sum = 0.0;
        h_sum += sampleAt(ivec2(stepX - (4.0 * uniforms.thickness), stepY));
        h_sum += sampleAt(ivec2(stepX - (3.0 * uniforms.thickness), stepY));
        h_sum += sampleAt(ivec2(stepX - (2.0 * uniforms.thickness), stepY));
        h_sum += sampleAt(ivec2(stepX - uniforms.thickness, stepY));
