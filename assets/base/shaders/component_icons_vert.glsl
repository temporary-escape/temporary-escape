...
void geometrize(vec4 pos) {
    ...
    // a: left-bottom
    if (gl_VertexIndex == 0) {
        vec2 va = pos.xy + vec2(-1.0, -1.0) * particleSize * dist + offset;
    ...
    // b: left-top
    if (gl_VertexIndex == 1) {
        vec2 vb = pos.xy + vec2(-1.0, 1.0) * particleSize * dist + offset;
    ...
    // d: right-bottom
    if (gl_VertexIndex == 2) {
        vec2 vd = pos.xy + vec2(1.0, -1.0) * particleSize * dist + offset;
    ...
    // c: right-top
    if (gl_VertexIndex == 3) {
        vec2 vc = pos.xy + vec2(1.0, 1.0) * particleSize * dist + offset;
