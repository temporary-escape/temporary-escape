...
MSGPACK_API_VERSION_NAMESPACE(MSGPACK_DEFAULT_API_NS) {
    ...
    template <> struct convert<Engine::AssetAsteroidPtr> {
        msgpack::object const& operator()(msgpack::object const& o, Engine::AssetAsteroidPtr& v) const {
            if (o.type != msgpack::type::STR)
                throw msgpack::type_error();
            std::string name;
            o.convert(name);
            v = Engine::AssetAsteroid::from(name);
            return o;
        }
    };

    template <> struct pack<Engine::AssetAsteroidPtr> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, Engine::AssetAsteroidPtr const& v) const {
