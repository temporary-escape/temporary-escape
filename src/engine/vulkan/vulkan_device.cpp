...
void VulkanDevice::blitImage(VulkanTexture& source, VulkanTexture& target, const BlitImageInfo& blitInfo,
                             const VkFilter filter) {

    VezImageBlit vezImageBlit = {};

    vezImageBlit.srcSubresource.mipLevel = blitInfo.src.mipLevel;
    vezImageBlit.srcSubresource.baseArrayLayer = blitInfo.src.baseArrayLayer;
    vezImageBlit.srcSubresource.layerCount = blitInfo.src.layerCount;
    vezImageBlit.srcOffsets[0].x = blitInfo.src.offset.x;
    vezImageBlit.srcOffsets[0].y = blitInfo.src.offset.x;
    vezImageBlit.srcOffsets[0].z = blitInfo.src.offset.x;
    vezImageBlit.srcOffsets[1].x = blitInfo.src.size.x;
    vezImageBlit.srcOffsets[1].y = blitInfo.src.size.y;
    vezImageBlit.srcOffsets[1].z = blitInfo.src.size.z;

    vezImageBlit.dstSubresource.mipLevel = blitInfo.dst.mipLevel;
    vezImageBlit.dstSubresource.baseArrayLayer = blitInfo.dst.baseArrayLayer;
    vezImageBlit.dstSubresource.layerCount = blitInfo.dst.layerCount;
    vezImageBlit.dstOffsets[0].x = blitInfo.dst.offset.x;
    vezImageBlit.dstOffsets[0].y = blitInfo.dst.offset.x;
    vezImageBlit.dstOffsets[0].z = blitInfo.dst.offset.x;
    vezImageBlit.dstOffsets[1].x = blitInfo.dst.size.x;
    vezImageBlit.dstOffsets[1].y = blitInfo.dst.size.y;
    vezImageBlit.dstOffsets[1].z = blitInfo.dst.size.z;

    vezCmdBlitImage(source.getHandle(), target.getHandle(), 1, &vezImageBlit, filter);
}
