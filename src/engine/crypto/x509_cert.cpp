...
std::string X509Cert::certFromAsioContext(asio::ssl::verify_context& ctx) {
...
X509Cert::X509Cert(std::string raw) : raw{std::move(raw)} {
    auto bio = std::shared_ptr<BIO>(BIO_new(BIO_s_mem()), [](BIO* b) { BIO_free(b); });
...
X509Cert::X509Cert(asio::ssl::verify_context& ctx) : X509Cert{certFromAsioContext(ctx)} {
}

X509Cert::X509Cert(const PrivateKey& pkey) {
...
std::string X509Cert::getSubjectName() const {
    if (!x509) {
        return "";
    }
