#include "shader_component_planet_surface.hpp"

using namespace Engine;

ShaderComponentPlanetSurface::ShaderComponentPlanetSurface(const Config& config, VulkanRenderer& vulkan,
    ...
    directionalLightsBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    directionalLightsBinding.pImmutableSamplers = nullptr;
    directionalLightsBinding.stageFlags =
        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_GEOMETRY_BIT;
    ...
    descriptorSetLayout = vulkan.createDescriptorSetLayout({
        cameraLayoutBinding,
        directionalLightsBinding,
        textureLayoutBinding,
    });

    auto& vert = modules.findByName("component-planet-surface.vert");
    auto& frag = modules.findByName("component-planet-surface.frag");
