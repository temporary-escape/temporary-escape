#include "shader_pass_bloom_combine.hpp"

using namespace Engine;

ShaderPassBloomCombine::ShaderPassBloomCombine(const Config& config, VulkanRenderer& vulkan, ShaderModules& modules,
                                               VulkanRenderPass& renderPass) {

    VkDescriptorSetLayoutBinding textureForwardLayoutBinding{};
    textureForwardLayoutBinding.binding = 0;
    textureForwardLayoutBinding.descriptorCount = 1;
    textureForwardLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    textureForwardLayoutBinding.pImmutableSamplers = nullptr;
    textureForwardLayoutBinding.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

    VkDescriptorSetLayoutBinding textureBlurredLayoutBinding{};
    textureBlurredLayoutBinding.binding = 1;
    textureBlurredLayoutBinding.descriptorCount = 1;
    textureBlurredLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    textureBlurredLayoutBinding.pImmutableSamplers = nullptr;
    textureBlurredLayoutBinding.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

    descriptorSetLayout = vulkan.createDescriptorSetLayout({
        textureForwardLayoutBinding,
        textureBlurredLayoutBinding,
    });

    auto& vert = modules.findByName("pass-bloom-combine.vert");
    auto& frag = modules.findByName("pass-bloom-combine.frag");
