...
VgWindow::VgWindow(const Engine::Config& config) : currentWindowSize{config.windowWidth, config.windowHeight} {
...
std::vector<const char*> VgWindow::getRequiredExtensions() {
...
VkSurfaceKHR VgWindow::createSurface(VkInstance instance) {
...
Vector2i VgWindow::getFramebufferSize() {
    int width, height;
    glfwGetFramebufferSize(window.get(), &width, &height);
    return {width, height};
}

void VgWindow::waitUntilValidFramebufferSize() {
...
void VgWindow::mouseMovedCallback(GLFWwindow* window, double x, double y) {
    auto& self = *static_cast<VgWindow*>(glfwGetWindowUserPointer(window));
    self.mousePos = {static_cast<int>(x), static_cast<int>(y)};
    self.eventMouseMoved(self.mousePos);
}

void VgWindow::mouseButtonCallback(GLFWwindow* window, int button, int action, int mods) {
    (void)mods;
...
void VgWindow::mouseScrollCallback(GLFWwindow* window, double xscroll, double yscroll) {
    auto& self = *static_cast<VgWindow*>(glfwGetWindowUserPointer(window));

    self.eventMouseScroll(static_cast<int>(xscroll), static_cast<int>(yscroll));
}

void VgWindow::keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    (void)scancode;
    (void)mods;
...
void VgWindow::windowSizeCallback(GLFWwindow* window, int width, int height) {
    auto& self = *static_cast<VgWindow*>(glfwGetWindowUserPointer(window));
...
void VgWindow::charCallback(GLFWwindow* window, unsigned int codepoint) {
    auto& self = *static_cast<VgWindow*>(glfwGetWindowUserPointer(window));
