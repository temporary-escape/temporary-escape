...
namespace Scissio {
class SCISSIO_API AssetPlanet : public Asset {
public:
    struct Definition {
        struct Surface {
...
MSGPACK_API_VERSION_NAMESPACE(MSGPACK_DEFAULT_API_NS) {
    ...
    template <> struct convert<Scissio::AssetPlanetPtr> {
        msgpack::object const& operator()(msgpack::object const& o, Scissio::AssetPlanetPtr& v) const {
            if (o.type != msgpack::type::STR)
                throw msgpack::type_error();
            std::string name;
            o.convert(name);
            v = Scissio::AssetPlanet::from(name);
            return o;
        }
    };

    template <> struct pack<Scissio::AssetPlanetPtr> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, Scissio::AssetPlanetPtr const& v) const {
