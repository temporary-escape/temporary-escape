    ...
    try {
        ...
        for (const auto& part : object.value().mesh.value().primitives) {
            ...
            if (material.baseColorTexture.has_value()) {
                primitive.material.baseColorTexture = findTextureOrNull(material.baseColorTexture.value().getUri());
            }

            if (material.normalTexture.has_value()) {
                primitive.material.normalTexture = findTextureOrNull(material.normalTexture.value().getUri());
            }

            if (material.emissiveTexture.has_value()) {
                primitive.material.emissiveTexture = findTextureOrNull(material.emissiveTexture.value().getUri());
            }

            if (material.metallicRoughnessTexture.has_value()) {
                primitive.material.metallicRoughnessTexture =
                    findTextureOrNull(material.metallicRoughnessTexture.value().getUri());
            }

            if (material.ambientOcclusionTexture.has_value()) {
                primitive.material.ambientOcclusionTexture =
                    findTextureOrNull(material.ambientOcclusionTexture.value().getUri());
            }
...
MSGPACK_UNPACK_FUNC(ModelPtr) {
    if (o.type == msgpack::type::STR) {
        const auto name = o.as<std::string>();
        v = AssetManager::singleton().find<Model>(name);
    } else {
