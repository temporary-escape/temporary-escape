...
namespace Scissio {
class SCISSIO_API AssetImage : public Asset {
public:
...
MSGPACK_API_VERSION_NAMESPACE(MSGPACK_DEFAULT_API_NS) {
    ...
    template <> struct convert<Scissio::AssetImagePtr> {
        msgpack::object const& operator()(msgpack::object const& o, Scissio::AssetImagePtr& v) const {
            if (o.type != msgpack::type::STR)
                throw msgpack::type_error();
            std::string name;
            o.convert(name);
            v = Scissio::AssetImage::from(name);
            return o;
        }
    };

    template <> struct pack<Scissio::AssetImagePtr> {
        template <typename Stream>
        msgpack::packer<Stream>& operator()(msgpack::packer<Stream>& o, Scissio::AssetImagePtr const& v) const {
