...
GuiContext::GuiContext(Canvas2D& canvas, const Config& config, AssetManager& assetManager)
    ...
    fontFaceRegular = assetManager.find<FontFace>(config.guiFontFaceRegular);
    fontFaceBold = assetManager.find<FontFace>(config.guiFontFaceBold);
...
void GuiContext::setFont(const FontFacePtr& fontFace, const float height) {
